class FleetManager {
    ...
    async initialize() {
        ...
        if (document.getElementById('fleet-view')) {
            this.renderFleetTab();
    ...
    async getAvailableVehiclesFromOffTechnicians(date) {
    ...
    async getRecentWorkOrders(hours = 24) {
        ...
        try {
            ...
            console.error("Error fetching recent fleet WOs:", error);
        }
        return recentWOs;
    }

    getVehicleByTechnician(technicianId) {
        return this.fleetData.find(vehicle => vehicle.assignedTo === technicianId) || null;
    }
    ...
    setupEventListeners() {
        ...
        fleetView.addEventListener('click', (event) => {
        ...
        fleetView.querySelector('#toggle-fleet-stats-btn')?.addEventListener('click', () => {
             fleetView.querySelector('#fleet-stats-container')?.classList.toggle('expanded');
        });
        fleetView.querySelector('#add-vehicle-btn')?.addEventListener('click', () => this.openVehicleProfileModal(null));
        fleetView.querySelector('#report-issue-btn')?.addEventListener('click', () => this.promptForVehicle('report-issue'));
        fleetView.querySelector('#monthly-inspection-btn')?.addEventListener('click', () => this.promptForVehicle('monthly-inspection'));
    }

    promptForVehicle(action) {
        const title = action === 'report-issue' ? 'Report an Issue' : 'Start Monthly Inspection';
        const buttonText = action === 'report-issue' ? 'Continue to Report' : 'Continue to Inspection';
        ...
        const modalBody = `
            <p>Please select the vehicle you want to ${action === 'report-issue' ? 'report an issue for' : 'inspect'}.</p>
    ...
    renderFleetDashboard() {
        ...
        document.getElementById('vansUnassigned').textContent = this.fleetData.filter(v => !v.assignedTo).length;
        document.getElementById('vansInRepairs').textContent = this.fleetData.filter(v => v.status === 'In Repairs').length;
    }
    ...
    async openVehicleProfileModal(vehicleId) {
        ...
        this.app.modalManager.showModal(`Vehicle Profile: ${vehicle.truckNumber || 'New Vehicle'}`, modalBody, [
            { text: 'Delete Vehicle', class: 'btn-danger', onclick: `laborTool.fleetManager.deleteVehicle('${vehicle.id}')`, disabled: isNew },
            { text: 'Cancel', class: 'btn-secondary', onclick: 'laborTool.modalManager.closeModal()' },
            { text: 'Save Changes', class: 'btn-primary', onclick: `laborTool.fleetManager.saveVehicleProfile('${vehicle.id}')` }
        ]);
    ...
    getVehicleDetailsFormHtml(vehicle) {
        ...
        allTechs.forEach(tech => techOptionsHtml += `<option value="${tech.id}" ${vehicle.assignedTo === tech.id ? 'selected' : ''}>${tech.name}</option>`);
        ...
        return `
            <div class="form-grid">
                ...
                <div class="form-group full-width"><label>Assigned To</label><select id="assignedTo" class="form-input">${techOptionsHtml}</select></div>
    ...
    async renderVehicleInspectionsList(vehicleId, containerId) {
        ...
        if (snapshot.empty) {
            ...
            content += snapshot.docs.map(doc => {
                ...
                return `<div class="history-item"><button class="btn-link" onclick="laborTool.fleetManager.renderSingleInspection('${vehicleId}', '${doc.id}')">Inspection from ${inspection.inspectionDate.toDate().toLocaleDateString()}</button></div>`;
            }).join('');
        }
        content += '</div></div>';
        container.innerHTML = content;
        ...
        let detailsHtml = `
            <div class="history-section">
                <div class="card-header" style="padding:0; margin-bottom:12px; border:none;">
                    <h4>Inspection on ${data.inspectionDate.toDate().toLocaleString()}</h4>
                    <button class="btn btn-secondary btn-small" onclick="laborTool.fleetManager.renderVehicleInspectionsList('${vehicleId}', 'inspections-tab')">Back to List</button>
                </div>
    ...
    async saveVehicleProfile(vehicleId) {
        ...
        const newTechId = document.getElementById('assignedTo').value;
    ...
    deleteVehicle(vehicleId) {
        ...
        this.app.modalManager.showConfirmDialog(
            ...
            `laborTool.fleetManager.confirmDeleteVehicle('${vehicleId}')`
        );
    }
    ...
    openWorkOrderModal(vehicleId) {
        ...
        this.app.modalManager.showModal(`New Work Order for ${vehicle.truckNumber}`, modalBody, [
            { text: 'Cancel', class: 'btn-secondary', onclick: 'laborTool.modalManager.closeModal()' },
            { text: 'Submit', class: 'btn-primary', onclick: `laborTool.fleetManager.submitWorkOrder('${vehicle.id}')` }
        ]);
    }
    ...
    openInspectionModal(vehicleId) {
        ...
        this.app.modalManager.showModal(`Monthly Inspection for ${vehicle.truckNumber}`, modalBody, [
            { text: 'Cancel', class: 'btn-secondary', onclick: 'laborTool.modalManager.closeModal()' },
            { text: 'Submit Inspection', class: 'btn-primary', onclick: `laborTool.fleetManager.submitInspection('${vehicle.id}')` }
        ]);
    }
    ...
    openRepairModal(vehicleId) {
        ...
        this.app.modalManager.showModal('Log Manual Repair', modalBody, [
            { text: 'Cancel', class: 'btn-secondary', onclick: 'laborTool.modalManager.closeModal()' },
            { text: 'Log Repair', class: 'btn-primary', onclick: `laborTool.fleetManager.logRepair('${vehicleId}')` }
        ]);
    }
