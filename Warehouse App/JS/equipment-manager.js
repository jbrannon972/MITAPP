class EquipmentManager {
    ...
    setupEventListeners() {
        document.getElementById('add-equipment-btn')?.addEventListener('click', () => this.openEquipmentModal());
        document.getElementById('scan-qr-code-btn')?.addEventListener('click', () => this.openQrScanner(this.handleHeaderScan.bind(this)));
        document.getElementById('check-grain-depression-btn')?.addEventListener('click', () => this.openQrScanner(this.handleGrainDepressionScan.bind(this)));
        document.getElementById('subtab-dashboard-btn')?.addEventListener('click', () => this.switchEquipmentView('dashboard'));
        document.getElementById('subtab-wos-btn')?.addEventListener('click', () => this.switchEquipmentView('work-orders'));
        
        document.getElementById('equipmentSearch')?.addEventListener('input', (e) => this.applyFilters(e.target.value));
    ...
    switchEquipmentView(viewName) {
        document.querySelectorAll('.equipment-view').forEach(v => v.classList.remove('active'));
        ...
        try {
            ...
            woSnapshot.forEach(doc => {
                if (doc.ref.parent.parent.parent.path === 'equipment') {
                    ...
                    content += `
                        <div class="wo-item">
                            <div class="wo-header" onclick="warehouseApp.equipmentManager.toggleWoDetails('${doc.id}')">
                            ...
                            <div class="wo-body" id="wo-body-${doc.id}">
                                ...
                                <div class="wo-card">
                                    ...
                                    <div class="wo-actions">
                                        <button class="btn btn-secondary" onclick="warehouseApp.equipmentManager.saveWoChanges('${equipmentId}', '${doc.id}')">Save Changes</button>
                                        <button class="btn btn-primary" onclick="warehouseApp.equipmentManager.completeWo('${equipmentId}', '${doc.id}')">Mark as Completed</button>
                                    </div>
                                </div>
                            </div>
                        </div>`;
    ...
    async openQrScanner(onSuccessCallback) {
        ...
        `;
        const footerButtons = [{ text: 'Cancel', class: 'btn-secondary', onclick: 'warehouseApp.equipmentManager.stopQrScanner(true)' }];
    ...
    promptToAddEquipment(qrCode) {
        ...
        this.app.modalManager.showModal('Equipment Not Found', message, [
            ...
            { text: 'Add New', class: 'btn-primary', onclick: `warehouseApp.equipmentManager.closeModal(); warehouseApp.equipmentManager.openEquipmentModal(null, '${qrCode}')` }
        ]);
    }
    ...
    renderDashboard() {
        ...
        summaryContainer.innerHTML = `
            <div id="filter-card-all" class="card metric-card single-line" onclick="warehouseApp.equipmentManager.resetFilters()">
                <div class="metric-header"><h3><i class="fas fa-boxes-stacked"></i> Total Units</h3></div><span class="stat-value">${total}</span>
            </div>
            <div id="filter-card-active" class="card metric-card single-line" onclick="warehouseApp.equipmentManager.toggleStatusFilter('active')">
                <div class="metric-header"><h3><i class="fas fa-check-circle"></i> Active</h3></div><span class="stat-value">${active}</span>
            </div>
            <div id="filter-card-maintenance" class="card metric-card single-line" onclick="warehouseApp.equipmentManager.toggleStatusFilter('maintenance')">
                <div class="metric-header"><h3><i class="fas fa-tools"></i> In Maintenance</h3></div><span class="stat-value">${inMaintenance}</span>
            </div>
            <div id="filter-card-retired" class="card metric-card single-line" onclick="warehouseApp.equipmentManager.toggleStatusFilter('retired')">
                <div class="metric-header"><h3><i class="fas fa-times-circle"></i> Retired</h3></div><span class="stat-value">${retired}</span>
            </div>
            <div id="filter-card-Dehumidifier" class="card metric-card single-line" onclick="warehouseApp.equipmentManager.toggleTypeFilter('Dehumidifier')">
                <div class="metric-header"><h3><i class="fas fa-wind"></i> Dehus</h3></div><span class="stat-value">${dehus}</span>
            </div>
            <div id="filter-card-Fan" class="card metric-card single-line" onclick="warehouseApp.equipmentManager.toggleTypeFilter('Fan')">
                <div class="metric-header"><h3><i class="fas fa-fan"></i> Fans</h3></div><span class="stat-value">${fans}</span>
            </div>
            <div id="filter-card-Air Scrubber" class="card metric-card single-line" onclick="warehouseApp.equipmentManager.toggleTypeFilter('Air Scrubber')">
    ...
    renderEquipmentAsCards(filteredData) {
        ...
        const cardsHtml = filteredData.map(item => {
            ...
            return `
                <div class="item-card" onclick="warehouseApp.equipmentManager.openEquipmentModal('${item.id}')">
    ...
    renderEquipmentAsTable(filteredData) {
        ...
        const rowsHtml = filteredData.map(item => `
            <tr onclick="warehouseApp.equipmentManager.openEquipmentModal('${item.id}')" style="cursor: pointer;">
                ...
                <td data-label="Actions" class="actions" style="text-align: right;">
                    <button class="btn btn-secondary btn-small" onclick="event.stopPropagation(); warehouseApp.equipmentManager.openEquipmentModal('${item.id}')">View/Edit</button>
                </td>
            </tr>
        `).join('');
    ...
    openGrainDepressionModal(item) {
        const modalBody = `
            <div class="grain-calc-grid">
                <div class="grain-calc-section"><h4>Inlet Readings</h4><div class="form-group"><label>Temp (°F)</label><input type="number" id="tempIn" class="form-input" oninput="warehouseApp.equipmentManager.updateGrainCalc()"></div><div class="form-group"><label>RH (%)</label><input type="number" id="rhIn" class="form-input" oninput="warehouseApp.equipmentManager.updateGrainCalc()"></div><p style="text-align:center; font-weight:bold;">GPP: <span id="gppInResult">0</span></p></div>
                <div class="grain-calc-section"><h4>Outlet Readings</h4><div class="form-group"><label>Temp (°F)</label><input type="number" id="tempOut" class="form-input" oninput="warehouseApp.equipmentManager.updateGrainCalc()"></div><div class="form-group"><label>RH (%)</label><input type="number" id="rhOut" class="form-input" oninput="warehouseApp.equipmentManager.updateGrainCalc()"></div><p style="text-align:center; font-weight:bold;">GPP: <span id="gppOutResult">0</span></p></div>
            </div>
            <div class="grain-results-display"><h3>Grain Depression</h3><div class="result-value" id="grainDepressionResult">0</div></div>`;
        const footerButtons = [{ text: "Cancel", class: "btn-secondary", onclick: "warehouseApp.closeModal()" }, { text: "Log Grain Depression", class: "btn-primary", onclick: `warehouseApp.equipmentManager.logGrainDepression('${item.id}')` }];
    ...
    async openEquipmentModal(equipmentId = null, qrCode = null) {
        ...
        const modalBody = `
            ...
            <div id="wo-tab" class="modal-tab-content"><div class="history-section"><h4>New Work Order</h4><div class="form-group"><label>Issue Description</label><textarea id="woDescription" class="form-input" rows="4"></textarea></div><button class="btn btn-primary" onclick="warehouseApp.equipmentManager.submitWorkOrder()">Submit WO</button></div><div class="history-section"><h4>Work Order History (${workOrders.length})</h4><div class="history-list">${workOrders.map(wo => `<div class="history-item"><strong>${wo.dateReported.toDate().toLocaleDateString()}:</strong> ${wo.issueDescription} <em>(Status: ${wo.status})</em></div>`).join("") || "<p>No work orders found.</p>"}</div></div></div>
            <div id="repair-tab" class="modal-tab-content"><div class="history-section"><h4>Log New Repair</h4><div class="form-group"><label>Repair Description</label><textarea id="repairDescription" class="form-input" rows="3"></textarea></div><button class="btn btn-primary" onclick="warehouseApp.equipmentManager.logRepair()">Log Repair</button></div><div class="history-section"><h4>Repair History (${repairHistory.length})</h4><div class="history-list">${repairHistory.map(r => `<div class="history-item"><strong>${r.dateCompleted.toDate().toLocaleDateString()}:</strong> ${r.repairDescription}</div>`).join("") || "<p>No repair history found.</p>"}</div></div></div>
            <div id="grain-tab" class="modal-tab-content"><div class="history-section"><h4>Grain Depression History (${grainHistory.length})</h4><div class="history-list">${grainHistory.map(g => `<div class="history-item"><strong>${g.loggedAt.toLocaleString()}:</strong> ${g.grainDepression.toFixed(2)} GPP</div>`).join("") || "<p>No history found.</p>"}</div></div></div>`;

        const footerButtons = [{ text: "Cancel", class: "btn-secondary", onclick: "warehouseApp.closeModal()" }, { text: "Save", class: "btn-primary", onclick: `warehouseApp.equipmentManager.saveEquipment()` }];
